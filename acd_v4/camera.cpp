...
static void * cam_stream(void *ptr) {
	u16 detect_score = 0, last_cx0 = 0, last_cy0 = 0, last_cx1 = 0,
			last_cy1 = 0;
...
u16 check_finger(u16 w, u16 h, u16 *c1, u8 *buff, u16 contrLimit,
	...
	*c1 = 0;
	...
	for (u16 i = 0; i < IH / 2; i++)
		vline[i] = buff[i * IW * 2 + IW / 2];

	hline = &buff[IS >> 1];

	cx = estimate_contrast(hline, IW);
	cy = estimate_contrast(vline, IH);

	if ((cx > contrLimit) && (cy > contrLimit)) {
...
//  	    	dy = - dy;
//
//  	    // проверка на изображения на стабильность
//  	    if ((dx < contrDelta)&&(dy < contrDelta))
//  	    {
		*c1 = sqrt(cx * cy);
		retval = 1;
//   	    }
